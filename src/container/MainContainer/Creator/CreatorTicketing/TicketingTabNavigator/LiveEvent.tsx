...
import {useGetLiveEventsQuery} from '@rtkServices/CreatorTicketingService';
...
const LiveEvents = ({searchQuery, onSearchChange}: LiveEventsProps) => {
  const navigation = useNavigation<any>();
  const {user} = useAppSelector((state: RootState) => state.user);
  const {data: liveEvents, isLoading} = useGetLiveEventsQuery({
    page: 1,
    limit: 20,
    creatorId: user?._id || '',
