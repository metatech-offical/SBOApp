...
import {CreatorNotificationSettingProps} from '@navigation/screens';
import {Colors} from '@constant/colors';
import {fonts} from '@constant/fontfamily';
import {fontSize} from '@constant/fontSize';
import {CreatorNotificationSettingData} from '@utils/data';
...
const CreatorNotificationSetting = ({
  navigation,
}: CreatorNotificationSettingProps) => {
  const {showError, showSuccess} = useToastMessage();
  const dispatch = useAppDispatch();
  const {user} = useAppSelector((state: RootState) => state.user);
  const [notificationReq] = useNotificationSettingsMutation();
  const [notifications, setNotifications] = useState(
    CreatorNotificationSettingData,
  );

  const initialNotification = () => {
    const userNotifications = user?.notificationSettings || {};
    const updated = CreatorNotificationSettingData?.map(item => ({
      ...item,
...
export default CreatorNotificationSetting;
